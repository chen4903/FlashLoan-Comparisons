...
interface IMorpho {
    event AccrueInterest(
        /* warning: missing UDVT support in source Solidity version; parameter is `Id`. */
        bytes32 indexed id,
    ...
    function extSloads(bytes32[] memory slots)
        external
        view
...
interface IBalancer_Vault {
    ...
    function batchSwap(
        uint8 kind,
        IBalancer_Vault_inner.BatchSwapStep[] memory swaps,
        address[] memory assets,
        IBalancer_Vault_inner.FundManagement memory funds,
    ...
    function exitPool(
        ...
        address sender,
        address recipient,
        IBalancer_Vault_inner.ExitPoolRequest memory request
    ) external;
    ...
    function joinPool(
        ...
        address sender,
        address recipient,
        IBalancer_Vault_inner.JoinPoolRequest memory request
    ) external payable;

    function managePoolBalance(IBalancer_Vault_inner.PoolBalanceOp[] memory ops) external;

    function manageUserBalance(IBalancer_Vault_inner.UserBalanceOp[] memory ops)
        external
        payable;
    ...
    function swap(
        IBalancer_Vault_inner.SingleSwap memory singleSwap,
        IBalancer_Vault_inner.FundManagement memory funds,
        uint256 limit,
        uint256 deadline
...
interface IBalancer_Vault_inner {
